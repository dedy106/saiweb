...
?>
    <div class="container-fluid mt-3">
        <div class="row" id="form-view">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Data Marketing
                ...
                <div class="card">
                    <div class="card-body">
                        <form class="form" id="form-tambah">
                            <h4 class="card-title mb-2">Input Marketing
                            ...
                            <div class="form-group row ">
								<label for="no_marketing" class="col-3 col-form-label">No Marketing</label>
                                <div class="col-9">
                                    <input class="form-control" type="text" placeholder="No Marketing" id="no_marketing" name="no_marketing">
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-9">
                                    <select name="flag_aktif" id="flag_aktif" class="form-control selectize">
                                        <!-- <option disabled hidden selected>--- Pilih Flag ---</option>
                                        <option value="1.AKTIF">Aktif</option>
                                        <option value="2.NONAKTIF">Nonaktif</option> -->
                                    </select>
                                </div>
                            </div>
                        </form>
   ...
   $(document).ready(function() {
        var select = $('#flag_aktif').selectize({
    ...
    $('#form-view').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Marketing.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'no_marketing':id,'kode_lokasi':kode_lokasi},
            success:function(result){
                if(result.status){
                    $('#id').val('edit');
                    $('#no_marketing').val(id);
                    $('#no_marketing').attr('readonly', true);
                    $('#nama').val(result.daftar[0].nama_marketing);
    ...
    var dataTable = $('#table-jenis').DataTable({
        ...
        'serverSide': true,
        'ajax': {
            'url': '<?=$root_ser?>/Marketing.php?fx=getView',
    ...
    $('#form-view').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, Dihapus!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '<?=$root_ser?>/Marketing.php',
                    dataType: 'json',
                    async:false,
                    data: {'no_marketing':kode,'kode_lokasi':kode_lokasi},
                    success:function(result){
                        if(result.status){
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Marketing.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Marketing.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
    ...
    $('#no_marketing,#nama').keydown(function(e){
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['no_marketing','nama'];
        if (code == 13 || code == 40) {
            e.preventDefault();
