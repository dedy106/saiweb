...
<!--                                 
    ...
    <div id='mySidepanel' class='sidepanel close'>
        ...
        <div class="row" style="margin-left: -5px;">
            <div class="col-sm-12">
                <div class="form-group" style='margin-bottom:0'>
                    <label for="jk-selectized">Jenis Kelamin</label>
...
<script type="text/javascript">
    ...
    function getPeserta(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/LapDaftarPeserta.php?fx=getPeserta',
            dataType: 'json',
            async:false,
    ...
    function getJK(no_peserta=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/LapDaftarPeserta.php?fx=getJK',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','no_peserta':no_peserta},
            success:function(result){    
                if(result.status){
                    var select = $('#jk').selectize();
                    select = select[0];
                    var control = select.selectize;
                    control.setValue(result.jk);
                }
            }
        });
    }
    ...
    $('.card-body').on('submit', '#formFilter', function(e){
        ...
        xurl ="<?=$root?>/app/dago/rptDaftarPeserta.php";
    ...
    $('#modalEmail').on('submit','#formEmail',function(e){
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/LapDaftarPeserta.php?fx=sendEmail',
            dataType: 'json',
            data: formData,
            async:false,
            contentType: false,
