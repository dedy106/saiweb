...
?>
    <div class="container-fluid mt-3">
        <div class="row" id="saku-datatable">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title mb-4"><i class='fas fa-cube'></i> Data Tipe Neraca 
                    ...
                    <div class="card-body table-responsive pt-0" style='height:450px'>
                            <form class="form" id="form-tambah" style='margin-bottom:100px'>
                                ...
                                <div class="form-group row">
                                    <label for="kode_tipe" class="col-3 col-form-label">Kode</label>
                                    <div class="col-3">
                                        <input class="form-control" type="text" placeholder="Kode Tipe" id="kode_tipe" name="kode_tipe" required >
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="nama_tipe" class="col-3 col-form-label">Nama</label>
                                    <div class="col-9">
                                        <input class="form-control" type="text" placeholder="Nama FS" id="nama_tipe" name="nama_tipe">
                                    </div>
                                </div>
                            </form>
                    </div>
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/TipeNeraca.php?fx=getEdit',
            dataType: 'json',
            async:false,
            ...
            success:function(result){
                if(result.status){
                    $('#id_edit').val('edit');
                    $('#kode_tipe').val(id);
                    $('#nama_tipe').val(result.daftar[0].nama_tipe);
                    $('#saku-datatable').hide();
                    $('#saku-form').show();
                }
            }
    ...
    $('#form-tambah').on('change', '#kode_tipe', function(){
        var id= $(this).val();
    ...
    var dataTable = $('#table-data').DataTable({
        ...
        'serverSide': true,
        'ajax': {
            'url': '<?=$root_ser?>/TipeNeraca.php?fx=getView',
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/TipeNeraca.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/TipeNeraca.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
    ...
    $('#kode_tipe,#nama_tipe').keydown(function(e){
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['kode_tipe','nama_tipe'];
        if (code == 13 || code == 40) {
            e.preventDefault();
