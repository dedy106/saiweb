...
<link href="<?=$folder_css?>/custom.css" rel="stylesheet">
    <div class="container-fluid mt-3">
        <div class="row" id="saku-datatable">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title mb-4"><i class='fas fa-cube'></i> Data Job User 
            ...
            <div class="col-sm-12" style="height: 90px;">
                <div class="card">
                    <div class="card-body pb-0">
                        <h4 class="card-title mb-4"><i class='fas fa-cube'></i> Form Job User
    ...
    function getProyek(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JobUser.php?fx=getProyek',
            dataType: 'json',
            async:false,
    ...
    function getKaryawan(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JobUser.php?fx=getKaryawan',
            dataType: 'json',
            async:false,
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JobUser.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'kode_job':id,'kode_lokasi':kode_lokasi,'nik':'<?=$nik?>'},
            success:function(result){
                if(result.status){
                    $('#id').val('edit');
                    $('#kode_job').val(id);
    ...
    var dataTable = $('#table-job').DataTable({
        ...
        'serverSide': true,
        'ajax': {
            'url': '<?=$root_ser?>/JobUser.php?fx=getJob',
            'data': {'kode_lokasi':kode_lokasi,'nik':'<?=$nik?>'},
            'async':false,
            'type': 'GET',
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '<?=$root_ser?>/JobUser.php',
                    dataType: 'json',
                    async:false,
                    data: {'kode_job':kode,'kode_lokasi':kode_lokasi,'nik':'<?=$nik?>'},
                    success:function(result){
                        if(result.status){
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/JobUser.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/JobUser.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
    ...
    $('#saku-datatable').on('click','#btn-history',function(e){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JobUser.php?fx=getHistory',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','kode_job':id,'nik':'<?=$nik?>'},
            success:function(result){    
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        var html='';
