...
?>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <button type="button" id="btn-form-tambah" class="btn btn-info ml-2" style="float:right;"><i class="fa fa-plus-circle"></i> Tambah Konten</button>
                                </h4>
                                                    ...
                                                    <div class="input-group">
                                                        <div class="input-group-prepend">
                                                            <div class="input-group-text">
                                                                <input type="radio" name="jenis" id="jen1">
                                                            </div>
                                                        </div>
                                                        <input type="text" class="form-control" aria-label="Text input with radio button" value="Artikel" readonly>
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="input-group">
                                                        <div class="input-group-prepend">
                                                            <div class="input-group-text">
                                                                <input type="radio" name="jenis" id="jen2">
                                                            </div>
                                                        </div>
                                                        <input type="text" class="form-control" aria-label="Text input with radio button" value="Video" readonly>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    $('#saku-data-konten').on('click', '#btn-form-tambah', function(){
        ...
        $('#tag').val('tag ke 1, yang tag 2, tag 3 ini');
    });
    ...
    //     var editor = new Quill('#editor', options);  // First matching element will be used
        ...
        $('#form-tambah-konten').on('submit','#form-tambah',function(e){
            e.preventDefault();
            var parameter = $('#id').val(); //untuk cek edit/save
            // var dari = untuk;//utk cek dari draft atau publish
            if(untuk == 'Draft'){ 
                //draft
                if(parameter == ''){
                    //save
                    var urlTuju = '<?=$root_ser?>/Konten.php?fx=insertKonten';
                }else{
                    //edit
                    var urlTuju = '<?=$root_ser?>/Konten.php?fx=updateKontenDraft';
                }
            }else{
                //publish
                if(parameter == ''){
                    //save
                    var urlTuju = '<?=$root_ser?>/Konten.php?fx=insertLangsung';
                }else{
                    //edit
                    var urlTuju = '<?=$root_ser?>/Konten.php?fx=updateKonten'
                }
            }
            ...
            // var tagnyayah = $('#tag').val();
            // var iniii = "[{\"value\":\"kampus\"},{\"value\":\"motor\"},{\"value\":\"listrik\"}]";
            // var cobayah = tagnyayah.split('value');
            // alert(cobayah);
            ...
            formData.append('jenisnya', jenisnya);
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if(res.status){
                    ...
                    if (res.daftar[0].jenis=='Artikel') {
                        $('#jen1').prop('checked', true);
                        $('#gambar').prop('disabled',false);
                        $('#tipe').prop('disabled',false);
                        $('#link').prop('disabled',true);
                        $('#jenis-gambar').show();
                        $('#jenis-video').hide();
                    } else if (res.daftar[0].jenis=='Video') {
                        $('#jen2').prop('checked', true);
                        $('#gambar').prop('disabled',true);
                        $('#link').prop('disabled',false);
                        $('#jenis-gambar').hide();
                        $('#jenis-video').show();
                    ...
                    var tag = res.daftar[0].tag;
                    var x = JSON.parse(tag);
                    console.log(x);
                    var finalArray = x.map(function (obj) {
                        return obj.value;
                    });
                    console.log(finalArray);
                    var isitags = '';
                    for(var a=0;a<finalArray.length;a++){
                        isitags += ","+finalArray[a];
                    }
                    console.log(isitags.substr(1));
                    $('#tag').val(isitags.substr(1));
                    $('#saku-data-konten').hide();
                    $('#form-tambah-konten').show();
                }
            },
