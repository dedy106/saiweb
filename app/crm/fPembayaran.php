            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs pull-right">
                        ...
                        <li class="pull-left header"><i class="fa fa-inbox"></i> Data Pembayaran</li>        
                    </ul>
                                ...
                                <div class="col-xs-12">
                                    <div class="box" style="border-top:none;">
                                        ...
                                        <div class="box-body sai-container-overflow-x">
                                            <table class='table table-bordered' id='table-Pembayaran' style='width:100%'>
                                                <thead>
                                                    <th>No Bukti</th>
                                                    <th>No Ref</th>
                                                    <th>Tanggal</th>
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="box" style="border-top:none;">
                                        ...
                                        <div class="box-body sai-container-overflow-x">
                                            <table class='table table-bordered' id='table-Pembayaran-keep' style='width:100%'>
                                                <thead>
                                                    <th>No Bukti</th>
                                                    <th>No Ref</th>
                                                    <th>Tanggal</th>
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="box" style="border-top:none;">
                                        ...
                                        <div class="box-body sai-container-overflow-x">
                                            <table class='table table-bordered' id='table-Pembayaran-finish' style='width:100%'>
                                                <thead>
                                                    <th>No Bukti</th>
                                                    <th>No Ref</th>
                                                    <th>Tanggal</th>
    ...
    <form id='web_form_insert' hidden enctype="multipart/form-data">
      <div class='row'>
        <div class='col-xs-12'>
          <div class='box'>
            <div class='box-header'>
              <h3 class="box-title" ><i class="fa fa-inbox"></i> Input Pembayaran</h3> 
          ...
          <div class='box box-warning'>
            <div class='box-body pad'> 
              ...
              <div class='row'>
                <div class='form-group'>
                  <label class='control-label col-sm-3'>Nilai Invoice/Kontrak</label>
                    <div class='col-sm-9' style='margin-bottom:5px;'>
                      <input type='text' name='nilai_invoice' id='nilai_invoice' placeholder='Masukkan Nilai' class='form-control currency input-form' required readonly>
                     </div>
                </div>
              </div>
              <div class='row'>
                <div class='form-group'>
                  <label class='control-label col-sm-3'>Nilai Bayar</label>
                    <div class='col-sm-9' style='margin-bottom:5px;'>
                      <input type='text' name='nilai' id='nilai' placeholder='Masukkan Nilai' class='form-control currency input-form' required>
                     </div>
                </div>
              </div>
              <div class='row'>
...
function getJenisDok(){
    $.ajax({
        type: 'GET',
        url: '<?=$root_ser?>/Pembayaran.php?fx=getJenisDok',
        dataType: 'json',
        data: {},
        success:function(result){    
            if(result.status){
...
function getSaldoInvoice(ref,no_bukti=null){
    $.ajax({
        type: 'GET',
        url: '<?=$root_ser?>/Pembayaran.php?fx=getSaldoInvoice&no_ref='+ref+'&no_bukti='+no_bukti,
        dataType: 'json',
        data: {},
        success:function(result){    
            if(result.status){
                $('#nilai_invoice').val(result.nilai);
            }
        }
    });
}
...
$(document).ready(function(){
  ...
  var dataTable = $('#table-Pembayaran').DataTable({
      'processing': true,
      'serverSide': true,
      'ajax': {
          'url': '<?=$root_ser?>/Pembayaran.php?fx=getDatatable',
          'data': {'jenis':'new'},
          'type': 'POST',
      ...
      'columnDefs': [
          {'targets': 8, 'className': 'text-center',data: null, 'defaultContent': action_html },{
                'targets': 7,
  ...
  var dataTable2 = $('#table-Pembayaran-keep').DataTable({
      'processing': true,
      'serverSide': true,
      'ajax': {
          'url': '<?=$root_ser?>/Pembayaran.php?fx=getDatatable',
      ...
      'columnDefs': [
          {'targets': 8, 'className': 'text-center', 'defaultContent': action_html2 },
          {
            'targets': 7,
            'className': 'text-right',
            'render': $.fn.dataTable.render.number( '.', ',', 0, 'Rp. ' )
  ...
  var dataTable3 = $('#table-Pembayaran-finish').DataTable({
      'processing': true,
      'serverSide': true,
      'ajax': {
          'url': '<?=$root_ser?>/Pembayaran.php?fx=getDatatable',
  ...
  $('#saiweb_container').on('click', '.web_datatable_edit', function(){
     ...
     var nilai = $(this).closest('tr').find('td:eq(7)').text();
  ...
  e.preventDefault();
    ...
    var nilai = toNilai($('#nilai').val());
    var nilai_inv = toNilai($('#nilai_invoice').val());
    var sts = $('#status').val();
    if(nilai > nilai_inv){
        alert("nilai bayar tidak boleh melebihi nilai invoice !");
    }else if((nilai < nilai_inv) && (sts =='2')){
        alert("nilai bayar anda kurang dari nilai invoice, jika ingin mencicil tagihan harap pilih status keep");
    }else{
        if(nilai == nilai_inv){
