...
<link href="<?=$folder_css?>/custom.css" rel="stylesheet">
    ...
    function getPP(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/StatusSiswa.php?fx=getPP',
            dataType: 'json',
            async:false,
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        var tmp= $(this).closest('tr').find('td').eq(3).html();
        tmp = tmp.split("-");
        var kode_pp = tmp[0];

        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/StatusGuru.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'kode_status':id,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
            success:function(result){
                if(result.status){
                    $('#id_edit').val('edit');
                    $('#kode_status').val(id);
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, Dihapus!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '<?=$root_ser?>/StatusGuru.php',
                    dataType: 'json',
                    async:false,
                    data: {'kode_status':kode,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
                    success:function(result){
                        if(result.status){
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/StatusGuru.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/StatusGuru.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
