...
<link href="<?=$folder_css?>/custom.css" rel="stylesheet">
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Slot.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'kode_slot':id,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
            success:function(result){
                if(result.status){
                    $('#id_edit').val('edit');
                    $('#kode_slot').val(id);
                    ...
                    $('#jam1').val(result.daftar[0].jam1);
                    $('#jam2').val(result.daftar[0].jam2);
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, Dihapus!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '<?=$root_ser?>/Slot.php',
                    dataType: 'json',
                    async:false,
                    data: {'kode_slot':kode,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
                    success:function(result){
                        if(result.status){
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Slot.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
