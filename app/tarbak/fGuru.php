...
<link href="<?=$folder_css?>/custom.css" rel="stylesheet">
    ...
    var dataTable = $('#table-data').DataTable({
        ...
        'serverSide': true,
        'ajax': {
            'url': '<?=$root_ser?>/Guru.php?fx=getView',
    ...
    function getPP(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Guru.php?fx=getPP',
            dataType: 'json',
            async:false,
    ...
    function getNIKGuru(id=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Guru.php?fx=getNIKGuru',
            dataType: 'json',
            async:false,
    ...
    function getMatpel(id=null,param){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Guru.php?fx=getMatpel',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','kode_pp':id},
            success:function(result){    
    ...
    function getStatus(id=null,param){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Guru.php?fx=getStatus',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','kode_pp':id},
            success:function(result){    
                ...
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        for(i=0;i<result.daftar.length;i++){
                            control.addOption([{text:result.daftar[i].kode_status + ' - ' + result.daftar[i].nama, value:result.daftar[i].kode_status}]);
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Guru.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'nik_guru':id,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
            success:function(result){
                if(result.status){
                    $('#id').val('edit');
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, Dihapus!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '<?=$root_ser?>/Guru.php',
                    dataType: 'json',
                    async:false,
                    data: {'nik_guru':kode,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
                    success:function(result){
                        if(result.status){
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Guru.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
