...
<link href="<?=$folder_css?>/custom.css" rel="stylesheet">
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Matpel.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'kode_matpel':id,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
            success:function(result){
                if(result.status){
                    $('#id').val('edit');
                    $('#kode_matpel').val(id);
                    ...
                    $('#sifat')[0].selectize.setValue(result.daftar[0].sifat);
    ...
    $('#saku-datatable').on('click','#btn-delete',function(e){
        ...
        confirmButtonText: 'Yes, Dihapus!'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'DELETE',
                    url: '<?=$root_ser?>/Matpel.php',
                    dataType: 'json',
                    async:false,
                    data: {'kode_matpel':kode,'kode_lokasi':kode_lokasi,'kode_pp':kode_pp},
                    success:function(result){
                        if(result.status){
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Matpel.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Matpel.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
