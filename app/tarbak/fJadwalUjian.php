...
<link href="<?=$folder_css?>/custom.css" rel="stylesheet">
    <div class="container-fluid mt-3">
        <div class="row" id="saku-datatable">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title mb-4"><i class='fas fa-cube'></i> Data Jadwal Ujian 
            ...
            <div class="col-sm-12" style="height: 90px;">
                <div class="card">
                    <div class="card-body pb-0">
                        <h4 class="card-title mb-4"><i class='fas fa-cube'></i> Form Jadwal Ujian
                    ...
                    <div class="card-body table-responsive pt-0" style='height:460px'>
                        <form class="form" id="form-tambah" style='margin-bottom:10px'>
                            ...
                            <div class="form-group row">
                                <label for="kode_sem" class="col-2 col-form-label">Semester</label>
                                <div class="col-3">
                                ...
                                <label for="kode_tingkat" class="col-2 col-form-label">Tingkat</label>
                                <div class="col-3">
                            ...
                            <div class="form-group row">
                                <label for="kode_jenis" class="col-2 col-form-label">Jenis Ujian</label>
                                <div class="col-3">
                                ...
                                <table class="table table-striped table-bordered table-condensed" id="input-grid" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th width="5%"><button type="button" style='padding:0' href="#" id="add-row" class="btn btn-default"><i class="fa fa-plus-circle"></i></button></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
    ...
    function getPP(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JadwalUjian.php?fx=getPP',
            dataType: 'json',
            async:false,
    ...
    function getTA(id=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JadwalUjian.php?fx=getTA',
            dataType: 'json',
            async:false,
    ...
    function getTingkat(id=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JadwalUjian.php?fx=getTingkat',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','kode_pp':id},
            success:function(result){    
    ...
    function getJenis(id=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JadwalUjian.php?fx=getJenis',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','kode_pp':id},
            success:function(result){    
                var select = $('#kode_jenis').selectize();
                select = select[0];
                var control = select.selectize;
                var kode = control.getValue();
                control.clearOptions();
    ...
    function getMatpel(id=null,param){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JadwalUjian.php?fx=getMatpel',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','kode_pp':id},
            success:function(result){    
    ...
    $('#form-tambah').on('click', '#add-row', function(){
      ...
      var no=$('#input-grid .row-jadwal:last').index();
      no=no+2;
      var input = "";
      input += "<tr class='row-jadwal'>";
      input += "<td width='5%' class='no-jadwal'>"+no+"</td>";
      input += "<td width='20%'><input type='date' class='form-control inp-tgl ' name='tanggal[]'></td>";
      input += "<td width='20%'><input type='time' class='form-control inp-jam ' name='jam[]'></td>";
      ...
      $('#input-grid tbody tr:last').find('.inp-tgl').focus();
  });
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/JadwalUjian.php?fx=getEdit',
            dataType: 'json',
            async:false,
            data: {'id_pekerjaan':id,'kode_lokasi':kode_lokasi},
            success:function(result){
                if(result.status){
                    $('#id').val('edit');
                    $('#id_pekerjaan').val(id);
    ...
    var dataTable = $('#table-data').DataTable({
        ...
        'serverSide': true,
        'ajax': {
            'url': '<?=$root_ser?>/JadwalUjian.php?fx=getView',
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/JadwalUjian.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/JadwalUjian.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
