    ...
    <div class="row" style="">
        <div style="z-index: 1;position: fixed;right: auto;left: auto;margin-right: 15px;margin-left: 25px;margin-top:15px" class="col-sm-12" id="subFixbar">
            <div class="card " id="sai-rpt-filter-box;" style="padding:10px;">
                ...
                <h4 class="card-title pl-1"><i class='fas fa-file'></i> Laporan Penjualan</h4>
                <hr>
                    <form id="formFilter">
...
<script type="text/javascript">
    ...
    function getPeriode(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Laporan.php?fx=getPeriode2',
            dataType: 'json',
            async:false,
    ...
    function getNIK(periode=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Laporan.php?fx=getNIK',
            dataType: 'json',
            async:false,
    ...
    function getNoBukti(periode=null,nik=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Laporan.php?fx=getNoBukti',
            dataType: 'json',
            async:false,
            ...
            success:function(result){    
                if(result.status){
                    ...
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        for(i=0;i<result.daftar.length;i++){
                            control.addOption([{text:result.daftar[i].no_jual+'-'+result.daftar[i].keterangan, value:result.daftar[i].no_jual}]);
                        }
                    }
                }
            }
    ...
    $('.card-body').on('submit', '#formFilter', function(e){
        ...
        xurl ="<?=$root?>/app/kasir/rptPnj3.php";
    ...
    $('#modalEmail').on('submit','#formEmail',function(e){
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/Laporan.php?fx=sendEmail',
            dataType: 'json',
            data: formData,
            async:false,
            contentType: false,
