<?php
    ...
    $exec=array();
    $sql = "exec sp_brg_hpp '".date('Ym')."','$kode_lokasi','$nik' ";
    array_push($exec,$sql);
    $sql2 ="exec sp_brg_stok_hpp2 '".date('Ym')."','$kode_lokasi','$nik'";
    array_push($exec,$sql2);
    $ress = executeArray($exec,$err);
    if($err == null){
        echo "";
    }else{
                ...
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        ...
                        for(i=0;i<result.daftar.length;i++){
                            ...
                            $dtBrg[result.daftar[i].kode_barang] = {harga:result.daftar[i].harga_seb,satuan:result.daftar[i].satuan,kode_akun:result.daftar[i].kode_akun,saldo:result.daftar[i].saldo,hrgjual:result.daftar[i].harga};  
                            $dtBrg2[result.daftar[i].barcode] = {harga:result.daftar[i].harga_seb,nama:result.daftar[i].nama,kd_barang:result.daftar[i].kode_barang,satuan:result.daftar[i].satuan,kode_akun:result.daftar[i].kode_akun,saldo:result.daftar[i].saldo,hrgjual:result.daftar[i].harga};
                        }
