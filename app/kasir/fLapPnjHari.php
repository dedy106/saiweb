    ...
    <div class="row" style="">
        <div style="z-index: 1;position: fixed;right: auto;left: auto;margin-right: 15px;margin-left: 25px;margin-top:15px" class="col-sm-12" id="subFixbar">
            <div class="card " id="sai-rpt-filter-box;" style="padding:10px;">
                ...
                <h4 class="card-title pl-1"><i class='fas fa-file'></i> Laporan Penjualan Per Hari</h4>
                <hr>
                    <form id="formFilter">
...
<script type="text/javascript">
    ...
    function getPeriode(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/LapPnjHari.php?fx=getPeriode2',
            dataType: 'json',
            async:false,
    ...
    function getNIK(periode=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/LapPnjHari.php?fx=getNIK',
            dataType: 'json',
            async:false,
    ...
    function getTanggal(periode=null,nik=null){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/LapPnjHari.php?fx=getTanggal',
            dataType: 'json',
            async:false,
            data: {'kode_lokasi':'<?=$kode_lokasi?>','periode':periode,'nik_kasir':nik},
            success:function(result){    
                if(result.status){
                    var select = $('#tanggal').selectize();
                    select = select[0];
                    var control = select.selectize;
                    control.clearOptions();
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        for(i=0;i<result.daftar.length;i++){
                            control.addOption([{text:result.daftar[i].tanggal, value:result.daftar[i].tanggal}]);
                        }
                    }
                }
            }
    ...
    $('.card-body').on('submit', '#formFilter', function(e){
        ...
        xurl ="<?=$root?>/app/kasir/rptPnjHari.php";
    ...
    $('#modalEmail').on('submit','#formEmail',function(e){
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/LapPnjHari.php?fx=sendEmail',
            dataType: 'json',
            data: formData,
            async:false,
            contentType: false,
