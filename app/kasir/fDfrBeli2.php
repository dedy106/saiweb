...
$(document).ready(function(){
                ...
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        ...
                        for(i=0;i<result.daftar.length;i++){
                            control.addOption([{text:result.daftar[i].nama, value:result.daftar[i].kode_barang}]);
                            control2.addOption([{kd_barang:result.daftar[i].kode_barang, nama:result.daftar[i].nama,barcode:result.daftar[i].barcode}]);
                            
                            $dtBrg[result.daftar[i].kode_barang] = {harga:result.daftar[i].harga_seb,satuan:result.daftar[i].satuan,kode_akun:result.daftar[i].kode_akun};  
                            $dtBrg2[result.daftar[i].barcode] = {harga:result.daftar[i].harga_seb,nama:result.daftar[i].nama,kd_barang:result.daftar[i].kode_barang,satuan:result.daftar[i].satuan,kode_akun:result.daftar[i].kode_akun};
                        }
        ...
        $.ajax({
            ...
            success:function(res){
                if(res.status){
                    ...
                    $('#iniNoBukti').html(res.daftar[0].no_bukti+"<input type='hidden' value='"+res.daftar[0].no_bukti+"' name='no_beli' class='form-control' maxlength='200' readonly id='web_form_edit_no_beli'>");
                    $('#iniNIK').html(res.daftar[0].nik_user+"<input type='hidden' value='"+res.daftar[0].nik_user+"' name='nik_kasir' class='form-control' maxlength='200' readonly id='web_form_edit_nik'>");
                    $('#totrans').val(toRp(res.daftar[0].total));    
                    $('#todisk').val(toRp(res.daftar[0].diskon));    
                    $('#toppn').val(toRp(res.daftar[0].ppn));    
                    $('#no_faktur').val(res.daftar[0].no_dokumen);    
                    $('#kode_vendor')[0].selectize.setValue(res.daftar[0].kode_vendor);
                    var input=``;
                    var no=1;
    ...
    $('#web_form_edit').submit(function(e){
        ...
        if(totrans <= 0){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Pembelian.php?fx=update',
                dataType: 'json',
                data: formData,
                contentType: false,
                cache: false,
    ...
    $('#cetakBtn').click(function(){
        ...
        window.location.href='<?=$root_app?>/fNotaBeli2/?param='+no_bukti+'&pmb=1';
    });
