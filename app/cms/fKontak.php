...
?>
    <div class="container-fluid mt-3">
        <div class="row" id="saku-datatable">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title mb-4"><i class='fas fa-cube'></i> Data Kontak
                    ...
                    <div class="card-body table-responsive pt-0" style='height:460px'>
                            <form class="form" id="form-tambah" style='margin-bottom:100px'>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-9">
                                        <input class="form-control" type="text" placeholder="Masukkan Judul Kontak" id="judul" name="judul">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="keterangan" class="col-3 col-form-label">Isi</label>
                                    <div class="col-9">
                                        <textarea class="form-control" type="text" placeholder="Masukkan Isi Kontak" id="keterangan" name="keterangan"></textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="latitude" class="col-3 col-form-label">Latitude</label>
                                    <div class="col-3">
                                        <input class="form-control" type="text" placeholder="Masukkan Latitude Kontak" id="latitude" name="latitude">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="longitude" class="col-3 col-form-label">Longitude</label>
                                    <div class="col-3">
                                        <input class="form-control" type="text" placeholder="Masukkan Longitude Kontak" id="longitude" name="longitude">
                                    </div>
                                </div>
                            </form>
                    </div>
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/Kontak.php?fx=getEdit',
            dataType: 'json',
            async:false,
            ...
            success:function(result){
                if(result.status){
                    ...
                    $('#latitude').val(result.daftar[0].latitude);
                    $('#longitude').val(result.daftar[0].longitude);
                    $('#saku-datatable').hide();
                    $('#saku-form').show();
                }
            }
    ...
    var dataTable = $('#table-data').DataTable({
        ...
        'serverSide': true,
        'ajax': {
            'url': '<?=$root_ser?>/Kontak.php?fx=getView',
    ...
    e.preventDefault();
        ...
        if(parameter==''){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Kontak.php?fx=simpan',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Kontak.php?fx=ubah',
                dataType: 'json',
                data: formData,
                async:false,
                contentType: false,
