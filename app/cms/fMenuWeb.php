            ...
            <form id="sai-treegrid-modal-form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row" style=''>
                        <label for="lvl-set" class="col-3 col-form-label">Level</label>
                        <div class="col-9">
                            <input type='number' name='level_menu' maxlength='5' class='form-control' readonly required id='lv-set'> 
                        </div>
                    </div>
                    <div class="form-group row" style='display:none'>
                        <label for="nu-set" class="col-3 col-form-label">Urutan</label>
                        <div class="col-9">
                            <input type='text' name='nu' class='form-control' readonly required id='nu-set'>
                        </div>
                    </div>
                    <div class='form-group row' style='display:none'>
                        <label for="rowindex-set" class="col-3 col-form-label">Row index</label>
...
<script type="text/javascript">
    ...
    function init(kode_klp=null){
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/MenuWeb.php?fx=getMenu',
            dataType: 'json',
                ...
                if(result.status){
                    ...
                    if(typeof result.html !== 'undefined'){
                        var html = `<table class='treegrid table' id='sai-treegrid'>
                            <thead><th>Kode Menu</th><th>Nama Menu</th><th>Kode Form</th><th>Jenis</th></thead>
                            <tbody>
                            `+result.html+`
                            </tbody>
                        </table>`;
    ...
    function getKlp(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/MenuWeb.php?fx=getKlp',
            dataType: 'json',
    ...
    function getLink(){
        $.ajax({
            type: 'GET',
            url: '<?=$root_ser?>/MenuWeb.php?fx=getLink',
            dataType: 'json',
            data: {'kode_lokasi':'<?php echo $kode_lokasi ?>','kode_menu':'<?=$kode_klp?>'},
            success:function(result){    
                if(result.status){
    ...
    $(document).ready(function(){
        ...
        $('.sai-treegrid-btn-del').click(function(){
            if($(".ui-selected").length != 1){
                ...
                if(sts){
                    ...
                    // window.location = "<?=$root_ser?>/MenuWeb.php?fx=delMenu&param="+selected_id+"&lok="+lokasi;
                    var kode_klp=$('#kode_klp')[0].selectize.getValue();
                    $.ajax({
                        type: 'GET',
                        url: '<?=$root_ser?>/MenuWeb.php?fx=delMenu',
                        dataType: 'json',
        ...
        $('.sai-treegrid-btn-ub').click(function(){
            if($(".ui-selected").length == 1){
                ...
                var this_induk = $("."+node_class).find('.set_induk').val();
        ...
        $("#sai-treegrid-modal-form").on("submit", function(event){
            ...
            $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/MenuWeb.php?fx=simpanMenu',
                dataType: 'json',
                data: formData,
                contentType: false,
                cache: false,
    ...
    e.preventDefault();
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/MenuWeb.php?fx=simpanMove',
            dataType: 'json',
            data: formData,
            async:false,
            contentType: false,
