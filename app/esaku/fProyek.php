...
?>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <form class="form" id="form-tambah">
                                    ...
                                    <div class="form-group row">
                                        <label for="tgl-mul" class="col-2 col-form-label">Tanggal Mulai</label>
                                        <div class="col-10">
                                            <input class="form-control" type="date" name="" id="isiDate">
                                            <!-- <input class="form-control" type="text" placeholder="Ex : Tgl-mul tgl-mul tgl-mul" id="tgl-mul" name="tgl-mul" autocomplete="off"> -->
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="tgl-sel" class="col-2 col-form-label">Tanggal Selesai</label>
                                        <div class="col-10">
                                            <input class="form-control" type="date" name="" id="isiDate2">
                                            <!-- <input class="form-control" type="text" placeholder="Ex : Tgl-sel tgl-sel tgl-sel" id="tgl-sel" name="tgl-sel" autocomplete="off"> -->
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="file-dok" class="col-2 col-form-label">File Dok</label>
                                        <div class="col-10">
                                            <input class="form-control" type="text" placeholder="Ex : File-dok file-dok file-dok" id="file-dok" name="file-dok" autocomplete="off">
                                        </div>
                                    </div>
                                    <input type="button" id="btnTest" value="T E S T"><input type="button" id="btnTest2" value="T E S T 2">
                                </form>
                            </div>
                        </div>
                    </div>
      ...
      $('#form-tambah-konten').on('click', '#btn-form-kembali', function(){
        ...
        $('#saku-data-konten').on('click','#btn-draft',function(e){
            ...
            if (confirm('Apakah anda yakin ingin un-publish/draft konten ini ?')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Proyek.php?fx=draftKonten',
                    dataType: 'json',
                    data: {
                        'id':kode,
        ...
        $('#saku-data-konten').on('click','#btn-form-hapus',function(e){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '<?=$root_ser?>/Proyek.php?fx=deleteKonten',
                        dataType: 'json',
    ...
    $('#form-tambah-konten').on('click','#btn-pub',function(){
        ...
        $('#form-tambah').submit(function(e){
            ...
            if (parameter=='') {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Proyek.php?fx=insertLangsung',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Proyek.php?fx=updateKonten',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
    ...
    $('#form-tambah-konten').on('click','#btn-save',function(){
        ...
        $('#form-tambah').submit(function(e){
            ...
            if (parameter=='') {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Proyek.php?fx=insertKonten',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Proyek.php?fx=updateKontenDraft',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
    ...
    $('#saku-data-konten').on('click', '#btn-form-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/Proyek.php?fx=getEditData',
            dataType: 'json',
            ...
            success:function(res){
                ...
                if(res.status){
                    ...
                    $('#tgl-mul').val(res.daftar[0].tgl-mul);
                    $('#tgl-sel').val(res.daftar[0].tgl-sel);
                    $('#file-dok').val(res.daftar[0].file-dok);
                    $('#saku-data-konten').hide();
                    $('#form-tambah-konten').show();
                }
            },
    ...
    //   $.ajax({
    //         type: 'POST',
    //         url: '<?=$root_ser?>/Proyek.php?fx=getGaleri',
    //         dataType: 'json',
    ...
    function getKategori(){
        ...
        $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Proyek.php?fx=getKtg',
                dataType: 'json',
                data: {'kode_lokasi':'<?php echo $kode_lokasi ?>'},
                success:function(result){    
                    if(result.status){
    ...
    $('#btnTest').click(function(){
        var date = $('#isiDate').val().split("-");
        console.log(date, $('#isiDate').val())
        day = date[2];
        month = date[1];
        year = date[0];
    ...
    $('#btnTest2').click(function(){
        var date = $('#isiDate2').val().split("-");
        console.log(date, $('#isiDate2').val())
        day = date[2];
        month = date[1];
        year = date[0];
        alert(day+' - '+month+' - '+year);
