...
?>
                            ...
                            <div class="card-body">
                                <form class="form" id="form-tambah">
                                    ...
                                    <div class="form-group row">
                                        <label for="nama" class="col-2 col-form-label">Nama Perusahaan</label>
                                        <div class="col-10">
                                            <input class="form-control" type="text" placeholder="Ex : Perusahaan perusahaan perusahaan" id="nama" name="nama" autocomplete="off">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="alamat" class="col-2 col-form-label">Alamat</label>
                                        <div class="col-10">
                                            <input class="form-control" type="text" placeholder="Ex : Alamat alamat alamat" id="alamat" name="alamat" autocomplete="off">
                                        </div>
                                    </div>
                                    <div class="form-group row">
    ...
    var dataTable = $('#table-konten').DataTable({
      ...
      'serverSide': true,
      'ajax': {
        'url': '<?=$root_ser?>/Customer.php?fx=getCustomer',
      ...
      $('#form-tambah-konten').on('click', '#btn-form-kembali', function(){
        ...
        $('#saku-data-konten').on('click','#btn-draft',function(e){
            ...
            if (confirm('Apakah anda yakin ingin un-publish/draft konten ini ?')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Customer.php?fx=draftKonten',
                    dataType: 'json',
                    data: {
                        'id':kode,
        ...
        $('#saku-data-konten').on('click','#btn-form-hapus',function(e){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '<?=$root_ser?>/Customer.php?fx=deleteKonten',
                        dataType: 'json',
    ...
    $('#form-tambah-konten').on('click','#btn-pub',function(){
        ...
        $('#form-tambah').submit(function(e){
            ...
            if (parameter=='') {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Customer.php?fx=insertLangsung',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Customer.php?fx=updateKonten',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
    ...
    $('#form-tambah-konten').on('click','#btn-save',function(){
        ...
        $('#form-tambah').submit(function(e){
            ...
            if (parameter=='') {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Customer.php?fx=insertKonten',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?=$root_ser?>/Customer.php?fx=updateKontenDraft',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
    ...
    $('#saku-data-konten').on('click', '#btn-form-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/Customer.php?fx=getEditData',
            dataType: 'json',
            ...
            success:function(res){
                ...
                if(res.status){
                    ...
                    $('#alamat').val(res.daftar[0].alamat);
                    $('#pic').val(res.daftar[0].pic);
                    $('#no_telp').val(res.daftar[0].no_telp);
                    $('#email').val(res.daftar[0].email);
                    $('#gambar')[0].selectize.setValue(res.daftar[0].gambar);
                    $('#saku-data-konten').hide();
                    $('#form-tambah-konten').show();
                }
            },
    ...
    function getGaleri(){
      $.ajax({
            type: 'POST',
            url: '<?=$root_ser?>/Customer.php?fx=getGaleri',
            dataType: 'json',
    ...
    function getKategori(){
        ...
        $.ajax({
                type: 'POST',
                url: '<?=$root_ser?>/Customer.php?fx=getKtg',
                dataType: 'json',
                data: {'kode_lokasi':'<?php echo $kode_lokasi ?>'},
                success:function(result){    
                    if(result.status){
