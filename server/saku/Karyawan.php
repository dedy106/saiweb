<?php
    ...
    function getView(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $query .= "select nik,nama,email,no_telp from karyawan where kode_lokasi='".$kode_lokasi."'  ";
    ...
    function getEdit(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $sql="select nik,nama,alamat,jabatan,email,no_telp,kode_pp,npwp,bank,cabang,no_rek,nama_rek,status,grade,kota,kode_pos,no_hp,flag_aktif,foto,ttd,kode_unit,kode_jab,kode_jab2,nip,flag_sppd from karyawan where kode_lokasi='".$_GET['kode_lokasi']."' and nik='$id' ";
    ...
    function simpan(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            if(isUnik($_POST["nik"])){
                ...
                if(ISSET($_FILES["ttd"]["name"]) AND !empty($_FILES["ttd"]["name"])){
                    ...
                    $filepath2=$namaFileBaru;
                }else{
                    $filepath2="-";
                }

                $sql1= "insert into karyawan(nik,nama,alamat,jabatan,email,no_telp,kode_pp,npwp,bank,cabang,no_rek,nama_rek,status,grade,kota,kode_pos,no_hp,flag_aktif,foto,ttd,kode_unit,kode_jab,kode_jab2,nip,flag_sppd,kode_lokasi) values ('".$data['nik']."','".$data['nama']."','".$data['alamat']."','".$data['jabatan']."','".$data['email']."','".$data['no_telp']."','".$data['kode_pp']."','".$data['npwp']."','".$data['bank']."','".$data['cabang']."','".$data['no_rek']."','".$data['nama_rek']."','".$data['status']."','".$data['grade']."','".$data['kota']."','".$data['kode_pos']."','".$data['no_hp']."','".$data['flag_aktif']."','".$filepath."','".$filepath2."','".$data['kode_unit']."','".$data['kode_jab']."','".$data['kode_jab2']."','".$data['nip']."','".$data['flag_sppd']."','$kode_lokasi') ";
    ...
    function ubah(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            if(ISSET($_FILES["file_gambar"]["name"]) AND !empty($_FILES["file_gambar"]["name"])){
                ...
                $sql = "select foto from karyawan where  nik='".$data['nik']."' and kode_lokasi='".$data['kode_lokasi']."' ";
                $refoto = execute($sql);
            ...
            if(ISSET($_FILES["ttd"]["name"]) AND !empty($_FILES["ttd"]["name"])){
                ...
                $sql = "select ttd from karyawan where  nik='".$data['nik']."' and kode_lokasi='".$data['kode_lokasi']."' ";
                $refoto = execute($sql);
                $filepath2=$refoto->fields[0];
            }

            $del = "delete from karyawan where nik='".$data['nik']."' and kode_lokasi='".$data['kode_lokasi']."' ";
            array_push($exec,$del);
