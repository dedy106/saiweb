<?php
    ...
    function login(){
        ...
        $sql="select a.kode_menu_lab as kode_klp_menu, a.nik, a.nama, a.pass, a.status_admin, a.klp_akses, a.kode_lokasi,b.nama as nmlok, c.kode_pp,d.nama as nama_pp,
			b.kode_lokkonsol,d.kode_bidang, case when isnull(c.foto,'-') = '-' then isnull(f.foto,'-') else isnull(c.foto,'-') end as foto,isnull(e.form,'-') as path_view,b.logo
        from hakakses a 
        ...
        left join dgw_peserta f on a.nik=f.no_peserta and a.kode_lokasi=f.kode_lokasi 
		...
		if($jum > 0){
            try{
                ...
                if($rs2){
                    ...
                    // setcookie('namalokasi', $row->nmlokasi, time() + (86400 * 0.5), "/");				
                    ...
                    $_SESSION['exit_url']=$root_ser."/dago/cLogin.php?fx=logout";
                    // setcookie('exit_url', $root_ser."/dago/cLogin.php?fx=logout", time() + (86400 * 0.5), "/");	
                    ...
                    if($row->path_view != null || $row->path_view != "-" ){
                        ...
                        $dash="";
                        // header("Location: $root_app/dago/dashboard.php".$dash, true, 301);
                        
                    }
                   
                    //echo "$root_app/dago/dashboard.php".$dash;
                    header("Location: $root_app/dago_main/".$dash, true, 301);
                    // echo $root2."/fMain.php?hal=".$dash;
                    exit();
                }else{
                    echo "<script>alert('Error Login : ".$rs2."'); window.location='$root_app/dago';</script>";   
            ...
            echo "<script>alert('Username, password salah !'); window.location='$root_app/dago';</script>";
        }
