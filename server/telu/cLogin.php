<?php
    ...
    function login(){
        ...
        b.kode_lokkonsol,d.kode_bidang, isnull(c.foto,'-') as foto,isnull(e.form,'-') as path_view,b.logo,isnull(c.email,'-') as email,isnull(c.no_telp,'-') as no_telp,isnull(c.jabatan,'-') as jabatan
        from hakakses a 
		...
		if($jum > 0){
            try{
                ...
                if($rs2){
                    ...
                    $_SESSION['exit_url']=$root_ser."/telu/cLogin.php?fx=logout";
                    // setcookie('exit_url', $root_ser."/cms/cLogin.php?fx=logout", time() + (86400 * 0.5), "/");	
                    ...
                    if($row->path_view != null || $row->path_view != "-" ){
                        ...
                        $dash="";
                        // header("Location: $root_app/cms/dashboard.php".$dash, true, 301);
                        
                    }
                    $_SESSION['dash'] = $dash;
                   
                    //echo "$root_app/cms/dashboard.php".$dash;
                    // header("Location: $root_app/telu_main/".$dash, true, 301);
                    ...
                    echo json_encode($response);
                    // echo "<script>alert('Error Login : ".$rs2."'); window.location='$root_app/telu';</script>";   
            ...
            echo json_encode($response);
            // echo "<script>alert('Username, password salah !'); window.location='$root_app/telu';</script>";
        }
    ...
    function loadMenu(){
        ...
        $hasil='';
        $sql="select a.*,b.form from menu a left join m_form b on a.kode_form=b.kode_form where a.kode_klp = '$kodeklp' and (isnull(a.jenis_menu,'-') = '-' OR a.jenis_menu = '') order by kode_klp, rowindex";
        $rs=execute($sql);
        ...
        if(count($daftar_menu) >0){
            ...
            for($i=0; $i<count($daftar_menu); $i++){
                ...
                $hasil .="<a class='nav2-link a_link ".$aktif."' href='#' data-href='".$this_link."'><img src='$folder_assets/assets/icons/".$icon."' style='margin-bottom:5px;width:15px'>&nbsp; ".$daftar_menu[$i]['nama']."</a>";
