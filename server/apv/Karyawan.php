<?php
    ...
    function getKaryawan(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $kode_lokasi = $_GET['kode_lokasi'];
            $query .= "select nik,nama,kode_pp,kode_jab,email,no_telp from apv_karyawan where kode_lokasi='".$kode_lokasi."'  ";

            $column_array = array('nik','nama','kode_pp','kode_jab','email','no_telp');
    ...
    function getEdit(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $sql="select nik,nama,kode_pp,kode_jab,foto as file_gambar,email,no_telp from apv_karyawan where kode_lokasi='".$_GET['kode_lokasi']."' and nik='$id' ";
    ...
    function simpan(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            if(isUnik($_POST["nik"])){
                ...
                $sql1= "insert into apv_karyawan(nik,nama,kode_lokasi,kode_pp,kode_jab,foto,email,no_telp) values ('".$data['nik']."','".$data['nama']."','".$data['kode_lokasi']."','".$data['kode_pp']."','".$data['kode_jab']."','".$filepath."','".$data['email']."','".$data['no_telp']."') ";
    ...
    function ubah(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            if(ISSET($_FILES["file_gambar"]["name"]) AND !empty($_FILES["file_gambar"]["name"])){
                ...
                $sql = "select foto from apv_karyawan where  nik='".$data['nik']."' and kode_lokasi='".$data['kode_lokasi']."' ";
                $refoto = execute($sql);
                $filepath=$refoto->fields[0];
            }

            $del = "delete from apv_karyawan where nik='".$data['nik']."' and kode_lokasi='".$data['kode_lokasi']."' ";
            array_push($exec,$del);
