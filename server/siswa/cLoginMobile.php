<?php
    ...
    function login(){
        ...
        if($cek['kode_menu'] == "SISWAWEB" OR $cek['kode_menu'] == "SISWA"){
            $sql="select a.nik, a.kode_menu, a.kode_lokasi, a.kode_pp, b.nis, b.nama, a.foto, a.status_login, b.kode_kelas, isnull(e.form,'-') as path_view,x.nama as nama_pp,b.email
            from sis_hakakses a 
        ...
        if($rs->RecordCount() > 0){
            try{
                ...
                if($rs2){
                    ...
                    header("Location: $root_app/siswa_mainmobile/".$dash, true, 301);
                    // echo $root2."/fMain.php?hal=".$dash;
                    exit();
                }else{
                    echo "<script>alert('Error Login : ".$rs2."'); window.location='$root_app/siswa_mobile';</script>";   
            ...
            echo "<script>alert('Username, password salah !'); window.location='$root_app/siswa_mobile';</script>";
        }
    ...
    function logout(){
        ...
        $root_app=$_SERVER['REQUEST_SCHEME']."://".$_SERVER['SERVER_NAME'];
        $header="Location: $root_app/siswa_mobile";
