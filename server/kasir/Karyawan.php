<?php
    ...
    function getView(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            ...
            $query .= "select nik,nama,alamat,jabatan,no_telp,email,kode_pp from karyawan where kode_lokasi='".$kode_lokasi."'  ";

            $column_array = array('nik','nama','alamat','jabatan','no_telp','email','kode_pp');
            $order_column = 'ORDER BY nik '.$_POST['order'][0]['dir'];
    ...
    function getEdit(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            ...
            $sql="select nik,kode_lokasi,nama,alamat,jabatan,no_telp,email,kode_pp, status, no_hp,flag_aktif,foto from karyawan where kode_lokasi='".$_GET['kode_lokasi']."' and nik='$id' ";
    ...
    function simpan(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            if(isUnik($_POST["nik"])){
                ...
                $sql1= "insert into karyawan(nik,kode_lokasi,nama,alamat,jabatan,no_telp,email,kode_pp, status, no_hp,flag_aktif,foto) values ('".$data['nik']."','".$data['kode_lokasi']."','".$data['nama']."','".$data['alamat']."','".$data['jabatan']."','".$data['no_telp']."','".$data['email']."','".$data['kode_pp']."','-','".$data['no_hp']."','".$data['flag_aktif']."','".$filepath."') ";
    ...
    function hapus(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            ...
            $data = $_DELETE;
            $del = "delete from karyawan where nik='".$data['kode']."' and kode_lokasi='".$data['kode_lokasi']."' ";
            array_push($exec,$del);
