<?php
    ...
    function isUnik($isi){
        ...
        $schema = db_Connect();
        $auth = $schema->SelectLimit("SELECT kode_klp FROM menu_klp where kode_klp='$isi' ", 1);
        if($auth->RecordCount() > 0){
            return false;
        }else{
            return true;
    ...
    function getView(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            ...
            $query .= "select kode_klp,nama from menu_klp ";

            $column_array = array('kode_klp','nama');
    ...
    function getEdit(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            ...
            $sql="select kode_klp,nama from menu_klp where kode_klp='$id' ";
    ...
    function simpan(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            if(isUnik($_POST["kode_klp"])){
                ...
                $sql1= "insert into menu_klp(kode_klp,nama) values ('".$data['kode_klp']."','".$data['nama']."') ";
    ...
    function ubah(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            $exec = array();
            $del = "delete from menu_klp where kode_klp='".$data['kode_klp']."' ";
            array_push($exec,$del);
    ...
    function hapus(){
        ...
        if($_COOKIE['isLogedIn'] AND cekAuth($_COOKIE['userLog'])){
            ...
            $data = $_DELETE;
            $del = "delete from menu_klp where kode_klp='".$data['kode']."'";
            array_push($exec,$del);
