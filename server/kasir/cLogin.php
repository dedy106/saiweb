<?php
		...
		if($jum > 0){
            try{
                ...
                if($rs2){
                    ...
                    setcookie('isLogedIn', true, time() + (86400), "/");			
                    $_SESSION['userLog'] = $row->nik;
                    setcookie('userLog', $row->nik, time() + (86400), "/");	
                    $_SESSION['lokasi'] = $row->kode_lokasi;
                    setcookie('lokasi', $row->kode_lokasi, time() + (86400), "/");	
                    $_SESSION['kodeMenu'] = $row->kode_klp_menu;
                    setcookie('kodeMenu', $row->kode_klp_menu, time() + (86400), "/");	
                    $_SESSION['namalokasi'] = $row->nmlok;	
                    setcookie('namalokasi', $row->nmlok, time() + (86400), "/");				
                    $_SESSION['userStatus'] = $row->status_admin;
                    setcookie('userStatus', $row->status_admin, time() + (86400), "/");	
                    $_SESSION['namaUser'] = $row->nama;
                    setcookie('namaUser', $row->nama, time() + (86400), "/");	
                    $_SESSION['kodePP'] = $row->kode_pp;
                    setcookie('kodePP', $row->kode_pp, time() + (86400), "/");	
                    $_SESSION['namaPP'] = $row->nama_pp;
                    setcookie('namaPP', $row->nama_pp, time() + (86400), "/");	
                    $_SESSION['kodeLokasiKonsol']=$row->kodelokkonsol;
                    setcookie('kodeLokasiKonsol', $row->kodelokkonsol, time() + (86400), "/");	
                    $_SESSION['foto'] = $row->foto;
                    setcookie('foto', $row->foto, time() + (86400), "/");	
                    $_SESSION['dash'] = $row->path_view;
                    setcookie('dash', $row->path_view, time() + (86400), "/");	
                    $_SESSION['logo'] = $row->logo;	
                    setcookie('logo', $row->logo, time() + (86400), "/");	
                    $_SESSION['loginTime'] = date('d-m-Y');
                    setcookie('loginTime', date('d-m-Y'), time() + (86400), "/");	
                    $_SESSION['nikUser']= $row->nik."_".date('d-m-Y');	
                    setcookie('nikUser', $row->nik."_".date('d-m-Y'), time() + (86400), "/");				
                    $_SESSION['periode'] = $row1->periode;
                    setcookie('periode', $row1->periode, time() + (86400), "/");							
                    $_SESSION['userPwd']=$post['pass'];
                    setcookie('userPwd', $post['pass'], time() + (86400), "/");	
                    $_SESSION['exit_url']=$root_ser."/kasir/cLogin.php?fx=logout";
                    setcookie('exit_url', $root_ser."/kasir/cLogin.php?fx=logout", time() + (86400), "/");	
                    $_SESSION['form_login']="fLogin.php";
                    setcookie('form_login', "fLogin.php", time() + (86400), "/");	
                    $_SESSION['hakakses']="hakakses";
                    setcookie('hakakses', "hakakses", time() + (86400), "/");	
                    $_SESSION['api_key']=$new_key;
                    setcookie('api_key', $new_key, time() + (86400), "/");	

                    $cookie_name = "user";
                    $cookie_value = $row->nik;
                    setcookie($cookie_name, $cookie_value, time() + (86400), "/"); //12 jam
                    ...
                    setcookie('kode_fs', $row3->kode_fs, time() + (86400), "/");	
                    ...
                    if($row->path_view != null || $row->path_view != "-" ){
                        ...
                        $dash="";
                        // header("Location: $root_app/kasir/dashboard.php".$dash, true, 301);
                        
                    }
                   
                    //echo "$root_app/kasir/dashboard.php".$dash;
                    header("Location: $root_app/kasir_main/".$dash, true, 301);
                    // echo $root2."/fMain.php?hal=".$dash;
                    exit();
                }else{
                    echo "<script>alert('Error Login : ".$rs2."'); window.location='$root_app/kasir';</script>";   
            ...
            echo "<script>alert('Username, password salah !'); window.location='$root_app/kasir';</script>";
        }
        ...
        if($rs->RecordCount() > 0){
            try{
                ...
                if($rs2){
                    ...
                    // setcookie('kodeBidang', $row->kode_bidang, time() + (86400), "/");	
                    $_SESSION['foto'] = $row->foto;
                    setcookie('foto', $row->foto, time() + (86400), "/");	
                    $_SESSION['dash'] = $row->path_view;
                    setcookie('dash', $row->path_view, time() + (86400), "/");	
    ...
    function logout(){
        ...
        $root_app="http://".$_SERVER['SERVER_NAME'];
        $header="Location: $root_app/kasir";
        ...
        session_unset();
        session_destroy();
        setcookie("user", "", time() - (86400), "/"); //24 jam
        setcookie("isLogedIn", true, time() -(86400), "/");		
        setcookie('userLog', $row->nik, time() + -(86400), "/");
        setcookie('lokasi', $row->kode_lokasi, time() + -(86400), "/");	
        setcookie('kodeMenu', $row->kode_klp_menu, time() + -(86400), "/");
        ...
        unset($_COOKIE['isLogedIn']);
        setcookie("isLogedIn", NULL, -1);
        unset($_COOKIE['userLog']);
        setcookie("userLog", NULL, -1);
        unset($_COOKIE['lokasi']);
        setcookie("lokasi", NULL, -1);
        unset($_COOKIE['kodeMenu']);
        setcookie("kodeMenu", NULL, -1);
        unset($_COOKIE['namalokasi']);
        setcookie("namalokasi", NULL, -1);
        unset($_COOKIE['userStatus']);
        setcookie("userStatus", NULL, -1);
        unset($_COOKIE['namaUser']);
        setcookie("namaUser", NULL, -1);
        unset($_COOKIE['kodePP']);
        setcookie("kodePP", NULL, -1);
        unset($_COOKIE['namaPP']);
        setcookie("namaPP", NULL, -1);
        unset($_COOKIE['kodeLokasiKonsol']);
        setcookie("kodeLokasiKonsol", NULL, -1);
        // unset($_COOKIE['kodeBidang']);
        // setcookie("kodeBidang", NULL, -1);
        unset($_COOKIE['foto']);
        setcookie("foto", NULL, -1);
        unset($_COOKIE['dash']);
        setcookie("dash", NULL, -1);
        unset($_COOKIE['logo']);
        setcookie("logo", NULL, -1);
        unset($_COOKIE['loginTime']);
        setcookie("loginTime", NULL, -1);
        unset($_COOKIE['nikUser']);
        setcookie("nikUser", NULL, -1);
        unset($_COOKIE['periode']);
        setcookie("periode", NULL, -1);
        unset($_COOKIE['userPwd']);
        setcookie("userPwd", NULL, -1);
        unset($_COOKIE['exit_url']);
        setcookie("exit_url", NULL, -1);
        unset($_COOKIE['form_login']);
        setcookie("form_login", NULL, -1);
        unset($_COOKIE['hakakses']);
        setcookie("hakakses", NULL, -1);
        unset($_COOKIE['api_key']);
        setcookie("api_key", NULL, -1);
        unset($_COOKIE['kode_fs']);
        setcookie("kode_fs", NULL, -1);  
