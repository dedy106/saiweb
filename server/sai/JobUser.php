<?php
    ...
    function getKaryawan(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $sql="select nik,nama from sai_karyawan where kode_lokasi='$kode_lokasi' and nik='$nik' ";
            $rs=execute($sql);
            $response["daftar"]=array();
            while ($row = $rs->FetchNextObject(false)){
                $response['daftar'][] = (array)$row;
    ...
    function getJob(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $kode_lokasi = $_GET['kode_lokasi'];
            $nik = $_GET['nik'];
            $query .= "select kode_job,nama,no_proyek,nik,convert(varchar(10),tgl_mulai,103) as tgl_mulai,convert(varchar(10),tgl_selesai,103) as tgl_selesai,progress from sai_job_m where kode_lokasi='".$kode_lokasi."'  and nik='$nik' ";
    ...
    function getEdit(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $sql="select kode_job,nama,no_proyek,nik,convert(varchar(10),tgl_mulai,121) as tgl_mulai,convert(varchar(10),tgl_selesai,121) as tgl_selesai from sai_job_m where kode_lokasi='".$_GET['kode_lokasi']."' and kode_job='$id' and nik='$nik' ";
            
            $response['daftar'] = dbResultArray($sql);

            $sql2="select nu, kode_job, kode_lokasi, nik, nama, keterangan, status, tgl_input
            from sai_job_d where kode_lokasi='".$_GET['kode_lokasi']."' and kode_job='$id' and nik='$nik'  ";
    ...
    function ubah(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){
            ...
            $del = "delete from sai_job_m where kode_job='".$data['kode_job']."' and kode_lokasi='".$data['kode_lokasi']."' and nik='$nik' ";
            array_push($exec,$del);

            $del2 = "delete from sai_job_d where kode_job='".$data['kode_job']."' and kode_lokasi='".$data['kode_lokasi']."' and nik='$nik' ";
