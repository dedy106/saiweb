<?php
    ...
    function login(){
        ...
        $sql="select a.kode_klp_menu, a.nik, a.nama, a.pass, a.status_admin, a.klp_akses, a.kode_lokasi,b.nama as nmlok, c.kode_pp,d.nama as nama_pp,
			b.kode_lokkonsol,d.kode_bidang, f.foto,isnull(e.form,'-') as path_view,b.logo,f.email
        from hakakses a 
        ...
        left join sai_karyawan f on a.nik=f.nik and a.kode_lokasi=f.kode_lokasi 
        ...
        if($rs->RecordCount() > 0){
            try{
                ...
                if($rs2){
                    ...
                    $_SESSION['exit_url']=$root_ser."/belajar/Login.php?fx=logout";
                    ...
                    if($row->path_view != null || $row->path_view != "-" ){
                        ...
                        $dash="";
                        // header("Location: $root_app/sai/dashboard.php".$dash, true, 301);
                        
                    }
                   
                    
                    header("Location: $root_app/belajar_main/".$dash, true, 301);
                    // echo $root2."/fMain.php?hal=".$dash;
                    exit();
                }else{
                    echo "<script>alert('Error Login : ".$rs2."'); window.location='$root_app/belajar';</script>";   
            ...
            echo "<script>alert('Username, password salah !'); window.location='$root_app/belajar';</script>";
        }
    ...
    function logout(){
        ...
        $root_app=$_SERVER['REQUEST_SCHEME']."://".$_SERVER['SERVER_NAME'];
        $header="Location: $root_app/belajar";
